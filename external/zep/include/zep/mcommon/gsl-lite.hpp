...
#define gsl_ENABLE_IF_INTEGRAL_T(T)
    ...
    namespace details {
        ...
        template <
            class Container, class ElementType, class = typename std::enable_if<!details::is_span<Container>::value && !details::is_array<Container>::value && !details::is_std_array<Container>::value && std::is_convertible<typename std::remove_pointer<decltype(std::declval<Container>().data())>::type (*)[], ElementType (*)[]>::value>::type
    ...
    public:
        ...
        template <class Container, class = typename std::enable_if<
                                       details::can_construct_span_from<Container, element_type>::value>::type>
        gsl_api gsl_constexpr span(Container& cont)
            : first_(cont.data()), last_(cont.data() + cont.size()) {}
    ...
    public:
        ...
        template <
            class Container,
            class = typename std::enable_if<
                !details::is_std_array<Container>::value && !details::is_basic_string_span<Container>::value && std::is_convertible<typename Container::pointer, pointer>::value && std::is_convertible<typename Container::pointer, decltype(std::declval<Container>().data())>::value>::type>
        ...
        template <
            class Container,
            class = typename std::enable_if<
                !details::is_std_array<Container>::value && !details::is_basic_string_span<Container>::value && std::is_convertible<typename Container::pointer, pointer>::value && std::is_convertible<typename Container::pointer, decltype(std::declval<Container const&>().data())>::value>::type>
...
#if gsl_CPP11_OR_GREATER || gsl_COMPILER_MSVC_VERSION >= 120
        template <class U, class = typename std::enable_if<std::is_convertible<typename basic_string_span<U>::pointer, pointer>::value>::type>
